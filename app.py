...
class App():
    ...
    def add_url_to_logger(self) -> bool:
        ...
        with open(LOG_FILE, 'r+') as f:
            ...
            if json_str:
                ...
                json_data = {}
            if (json_data.get(self.domain_hash)) and  (json_data[self.domain_hash]['progress'] == 'running'):
                return False
    ...
    def start_bot(self):
        ...
        if is_validated:
            if self.add_url_to_logger():
                p = multiprocessing.Process(target=start_browser, args=(self.params, self.domain_hash))
                p.start()
            else:
                ui.notification('Archive is currently running', type='info', position='top')
    ...
    def parse_inputs(self):
        ...
        for x in self.params:
            if (x not in ['primary_keywords', 'secondary_keywords', 'oldest_date', 'earliest_date']):
                if (x == 'next_page_selector') and (self.params['pagination_type'] == 'infinite_scroll'):
                    pass
                else:
    ...
    def get_input(self, label: str, key: str, validation: dict|None = None, placeholder: str|None = None, width: str = 'w-5/6'):
        return ui.input(
                placeholder=placeholder,
                label=label, 
                validation=validation
                ).classes(width).bind_value(self, key)
            ...
            try:
                with ui.card().classes('w-full'):
                    if self.domain_hash:
                        ...
                        log_content = ''
                    ui.label(f'Log File of {self.base_url}').classes('text-lg font-bold')
                    ui.separator()
    ...
    def main_page_ui(self):
        ...
        with ui.element('div').classes('bg-grey-3 w-full'):
            with ui.column(align_items='center').classes('w-full content-center gap-5 p-2'):
                ...
                self.get_input('Next Page Selector', 'next_page_selector')
                self.get_input('Primary Keywords', 'primary_keywords', placeholder='Arrest;Bodycam')\
                .props("""hint='seperate multiple by ";"' autogrow""")
                self.get_input('Secondary Keywords', 'secondary_keywords', placeholder='Mother;Father')\
                .props("""hint='seperate multiple by ";"' autogrow""")
                with ui.element('div').classes('w-5/6 bg-yellow p-2'):
                    ui.label('Select Pagination type')
                    ui.radio(self.pagination_types).bind_value(self, 'pagination_type').props('inline')
                with ui.row().classes('w-5/6'):
                    self.get_input('Oldest Date', 'oldest_date', placeholder='January 12th 2024', width='w-2/5')
                    self.get_input('Earliest Date', 'earliest_date', placeholder='January 12th 2025', width='w-2/5')
                with ui.row().classes('w-5/6'):
        ...
        with ui.expansion(text='View Logs').classes('w-full bg-grey-2 pa-5'):
            self.get_input('Show logs of', 'base_url', placeholder='https://alachuachronicle.com/').on_value_change(
                lambda x: self.update_domain_hash()
            )
            self.log_ui()
            ui.button('Refresh Log', on_click=lambda: ui.timer(2, lambda: self.refresh_log()))
